...
This file specifies the Airlock/Gas hazard state machine, timing, and damage model for the Alien Base multiplayer map, inspired by the gas canister mechanics from Total War and Tank in Conker’s Bad Fur Day while remaining compatible with modern engines and a 16‑player cap. [conker.fandom](https://conker.fandom.com/wiki/Tank)

## Design Philosophy

The hazard system is a controllable “Tanks‑style” map event: rare, readable, and powerful, not a random battle royale storm. Players trigger events through consoles or objectives, which start a countdown; after a brief warning, lethal gas or vacuum floods low areas, forcing everyone onto catwalks and upper corridors like bunkers in the original Tank mode. [conker.fandom](https://conker.fandom.com/wiki/Canister)
...
- Minimal HUD dependency; players learn by sight and sound. [conker.fandom](https://conker.fandom.com/wiki/Tank)
...
- `hazard_hub_floor_gas`  
  ...
  - Effect: low‑lying toxic gas or vacuum damage; catwalk ring above this height is safe.
...
Exact shapes can be box, cylinder, or mesh‑based volumes depending on engine, as long as they match the design intent of “floor bad, high ground safe” during events. [conker.fandom](https://conker.fandom.com/wiki/Canister)
...
- `arming_duration_sec` = 5.0  
  - Mirrors the short warning countdown in the Tank canister mechanic before gas releases. [en.wikipedia](https://en.wikipedia.org/wiki/Conker's_Bad_Fur_Day)
...
- `damage_per_second_floor` = 60 HP/sec  
  - Two seconds in the volume should be almost lethal, imitating the “instantly kill all tanks” feel but giving infantry a tiny window to escape. [conker.fandom](https://conker.fandom.com/wiki/Tank)

- `damage_per_second_sublevel` = 90 HP/sec  
  - Faster kill to discourage camping the Bazooka tunnels.

You can scale absolute numbers by your engine’s default health values while preserving the relationships.
...
3. **Active → Cooldown**  
   ...
   - Actions:  
     ...
     - Venting FX and VO to indicate safe conditions.
...
- **Tick Damage:**  
  ...
  - Apply separately for floor and sublevel volumes.

- **Stacking:**  
  - If a character is simultaneously inside overlapping volumes (floor + sublevel), use the higher damage rate rather than stacking both, to keep balancing simpler.

- **Immunity / Gear:**  
  - Optionally allow certain pickups (e.g., a “Gas Mask” equivalent) to negate or reduce damage, mirroring the bunker/gas mask behavior in Original Bad Fur Day modes. [en.wikipedia](https://en.wikipedia.org/wiki/Conker's_Bad_Fur_Day)
...
- Sub‑level acid: lethal in ~1.1 seconds, intentionally punishing the heavy‑weapon camper.

These numbers echo the “instantly kill all tanks” tone of original gas events while respecting that players are now on foot and have less armor. [conker.fandom](https://conker.fandom.com/wiki/Canister)
...
- Per‑Trigger: Each trigger shares the same global cooldown; once one is used, all are disabled until the state machine returns to `Idle`.  
...
Core methods (to be mirrored in your engine):
...
- Score bonus to the instigator for kills caused during the active phase (similar to awarding the canister carrier’s team in N64 modes). [conker.fandom](https://conker.fandom.com/wiki/Tank)
...
To keep the experience aligned with N64 and early Live & Uncut intent: [en.wikipedia](https://en.wikipedia.org/wiki/Conker's_Bad_Fur_Day)
